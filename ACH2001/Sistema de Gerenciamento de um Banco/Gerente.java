...
public class Gerente extends Pessoa implements InterfaceGerente{
	...
	public boolean adicionarCliente(Cliente cliente) {
		...
		// Verificar se eh possivel adicionar um cliente
		if(numClientes < 20) {
			...
			for(int i = 0; i < numClientes; i++) {
				...
				/*	
				O cliente ja consta no arranjo de clientes (verifique isso usando o numero do cpf),
	 		        neste caso o cliente nao deve ser reinserido e o metodo deve retornar false
				*/
				if(cliente.cpf == this.clientes[i].cpf ) return false;
				
			}

			/*			
			o cliente passado como parametro nao consta no arranjo de clientes: o cliente 
	        	deve ser adicionado na posicao numClientes, o atributo numClientes deve ser 
	        	incrementado em 1 e o metodo deve retornar true
			*/
	...
	public void cobrarTodosEmprestimos() {
		...
		// Cobrar os emprestimos de todos os clientes do gerente atual
		for(int i = 0; i < this.numClientes; i++) {
			
			// Nao fazer nada para o cliente, caso seu valorDaDivida seja igual a zero
			if(this.clientes[i].getValorDaDivida() == 0) continue;
			
			// Verificar se o cliente consegue pagar
			if(this.clientes[i].getValorContaCorrente() >= this.clientes[i].getValorDaDivida()) {
				
				/*
				se o valorContaCorrente do cliente for maior ou igual ao valorDaDivida,
				deve fazer o cliente pagar a divida, isto e, o valorContaCorrente sera atualizado,
				descontando-se o valor da divida e o valorDaDivida sera zerado
				*/
				this.clientes[i].setValorContaCorrente(this.clientes[i].getValorContaCorrente() - this.clientes[i].getValorDaDivida());
				this.clientes[i].setValorDaDivida(0);
				...
				/*
				se o valorContaCorrente do cliente for menor do que o valorDaDivida, 
	    		        deve fazer o cliente pagar parte da divida, isto e, o valorDaDivida 
	           		sera atualizado, tendo seu valor diminuido pelo valorContaCorrente e 
	           		o valorContaCorrente sera zerado.
				*/
				this.clientes[i].setValorDaDivida(this.clientes[i].getValorDaDivida() - this.clientes[i].getValorContaCorrente());
				this.clientes[i].setValorContaCorrente(0);
